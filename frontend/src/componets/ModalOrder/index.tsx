  ...
  return(
   <Modal isOpen={isOpen} onRequestClose={onRequestClose} style={customStyles} >
    ...
    <div className={styles.container}>
      ...
      Mesa: <strong>{order[0].pedidos.table}</strong>
      </span>
      {order.map(item => (
        <section key={item.id} className={styles.containerItem}>
          <span>{item.amount} - <strong>{item.produto.name}</strong></span>
          <span className={styles.description}>{item.produto.description}</span>
        </section>
      ))}
